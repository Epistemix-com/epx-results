...
from ..utils import return_job_id, _path_to_results, _path_to_job, _path_to_run
...
def test_7(pkg_test_env):
    ...
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{1}"
    )

    p = _path_to_run(job_key="epx-results_simpleflu", run_id=2)
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{2}"
    )

    p = _path_to_run(job_key="epx-results_simpleflu", run_id=3)
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{3}"
    )
...
def test_9(pkg_test_env):
    ...
    p = _path_to_run(
        job_key="epx-results_simpleflu", run_id=1, FRED_RESULTS=FRED_RESULTS
    )
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{1}"
    )
    ...
    p = _path_to_run(
        job_key="epx-results_simpleflu", run_id=2, FRED_RESULTS=FRED_RESULTS
    )
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{2}"
    )
    ...
    p = _path_to_run(
        job_key="epx-results_simpleflu", run_id=3, FRED_RESULTS=FRED_RESULTS
    )
    assert p == os.path.join(
        PKG_FRED_RESULTS, "JOB", str(test_job_id), "OUT", f"RUN{3}"
    )
